...
module.exports = async  function (req, res) {
    ...
    if(companyDoc && companyDoc.length > 0 ) {
      for (let i=0 ; i < companyDoc.length ; i++) {
          ...
          if(userDoc) {
              let queryString = [];
              if(companyDoc[i].saved_searches && companyDoc[i].saved_searches.when_receive_email_notitfications === 'Never'
                  && companyDoc[i].last_email_sent  <  new Date(Date.now() + candidateSearch.convertToDays(companyDoc[i].saved_searches[0].when_receive_email_notitfications) * 24*60*60*1000)
              ) { // not sure about this
                  if (companyDoc[i].saved_searches && companyDoc[i].saved_searches.length > 0 && companyDoc[i].saved_searches[0].receive_email_notitfications === true) {
