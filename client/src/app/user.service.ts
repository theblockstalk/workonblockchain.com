...
export class UserService {
  ...
  create(user: User)
    ...
    admin_candidate_filter(is_approve : string , msg_tags : any,word:any, status:any)
    {
        return this.http.post<any>(URL+'users/admin_candidate_filter', { is_approve: is_approve , msg_tags : msg_tags, word : word, status:status}, {
            headers: new HttpHeaders().set('Authorization', this.token)
        })
            .map((res: Response) =>
            {
